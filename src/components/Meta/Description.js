...
class Description extends Component {
  ...
  get countyFragment() {
    ...
    const city =
        S(best[0].city).titleCase().s + `, ${best[0].USstate}`,
    ...
    return (
      <span>
        ...
        {jobFragment.length
          ...
          : 'for an H1B'}{' '}
        {this.yearFragment ? 'was' : 'is'} <b>{city}</b> with an
        average salary ${this.format(mean - countyMedian)} above the
        local household median. Median household income is a good
